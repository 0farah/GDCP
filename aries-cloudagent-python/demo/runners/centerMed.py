...
class CenterMedAgent(AriesAgent):
    def __init__(
        self,
        ident: str,
        http_port: int,
    ...
    async def handle_present_proof_v2_0(self, message):
        ...
        if state == "presentation-received":
            ...
            is_proof_of_consent = (
                pres_req["name"] == "Proof of Consent"
            )
            if is_proof_of_consent:
                log_status("#28.1 Received proof of consent, check claims")
...
async def main(args):
    centerMed_agent = await create_agent_with_args(args, ident="centerMed")

    try:
        log_status(
            "#1 Provision an agent and wallet, get back configuration details"
            + (
                f" (Wallet type: {centerMed_agent.wallet_type})"
                if centerMed_agent.wallet_type
                else ""
            )
        )
        agent = CenterMedAgent(
            "centerMed.agent",
            centerMed_agent.start_port,
            centerMed_agent.start_port + 1,
            genesis_data=centerMed_agent.genesis_txns,
            genesis_txn_list=centerMed_agent.genesis_txn_list,
            no_auto=centerMed_agent.no_auto,
            ...
            mediation=centerMed_agent.mediation,
            wallet_type=centerMed_agent.wallet_type,
        ...
        async for option in prompt_loop(options):
            ...
            if option is None or option in "xX":
                ...
                log_status("#20 Request proof of consent from centerInv")
                req_attrs = [
                    {
                        "name": "patient_ref",
        ...
        if centerMed_agent.show_timing:
            timing = await centerMed_agent.agent.fetch_timing()
            if timing:
                for line in centerMed_agent.agent.format_timing(timing):
                    log_msg(line)

    finally:
        terminated = await centerMed_agent.terminate()
